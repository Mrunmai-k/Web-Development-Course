...
app.get("/random", (req, res) => {
  const randomIndex = Math.floor(Math.random() * jokes.length); //This will get the random index.
  res.json(jokes[randomIndex]); // This will send the JS object of random index of jokes as a response in the json object.
});

//2. GET a specific joke
app.get("/jokes/:id", (req, res) => {
  const reqID = parseInt(req.params.id); //get id passed as parameter. /jokes/2/ and not as query /jokes?2
  const foundJoke = jokes.find((joke) => joke.id === reqID); // find joke id same as reqID
  res.json(foundJoke);
});
